...
log "Starting 50m k=2 learnable pooling (2B tokens) on 2 GPUs..."
...
log "Starting 50m k=2 word-boundary windows (2B tokens) on 2 GPUs..."
python -m torch.distributed.run --standalone --nproc_per_node=2 \
  experiments/chinchilla/train.py \
...
log "Starting 50m k=2 overlap w=4 s=2 (2B tokens) on 2 GPUs..."
python -m torch.distributed.run --standalone --nproc_per_node=2 \
  experiments/chinchilla/train.py \
...
log "Starting 50m k=2 exponential weights (2B tokens) on 2 GPUs..."
python -m torch.distributed.run --standalone --nproc_per_node=2 \
  experiments/chinchilla/train.py \
...
log "Starting 50m k=4 learnable pooling (4.07B tokens) on 2 GPUs..."
python -m torch.distributed.run --standalone --nproc_per_node=2 \
  experiments/chinchilla/train.py \
...
log "Starting 50m k=4 exponential weights (4.07B tokens) on 2 GPUs..."
python -m torch.distributed.run --standalone --nproc_per_node=2 \
  experiments/chinchilla/train.py \
